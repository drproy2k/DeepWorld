...
def augmentation_morpphed(NMNS_text, num, label):
    ...
    if num>7 : num=7  #속도가 너무 느려지는 것을 방지
...
for i in range(num_of_qsts) :
    ...
    for word_tag in morpphed_text:
        ...
        if (word_tag[1] in ['NNG', 'MAG', 'NNP', 'SL', 'VV+ETM', 'NP', 'VA+ETM', 'VV+ETM', 'XSN', 'MM', 'VV', 'VV+EC', 'VV+EP'] and len(word_tag[0]) >= 1):  # Check only Noun
            ...
            word_count += 1
            if abc_exist(word_tag[0]) == True: abc_exist_flag = True
    ...
    if abc_exist_flag == True:
        for word_tag in morpphed_text:
            # print(word_tag)
            if (word_tag[1] in ['NNG', 'MAG', 'NNP', 'SL', 'VV+ETM', 'NP', 'VA+ETM', 'VV+ETM', 'XSN', 'MM', 'VV',
                ...
                if abc_exist(word_tag[0]) == True:
                    ...
                    NMNS_text_en += word_tag[0] + ' '
                word_count_en += 1
                if word_tag[0].isalpha() == True: abc_exist_flag = True
        if word_count < 3:  # 모니터링
            print(morpphed_text)
            print(NMNS_text_en)
            print(word_count_en)
