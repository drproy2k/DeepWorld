...
class NERModel(object):
    ...
    def get_feed_dict(self, words, labels=None, lr=None, dropout=None):
        ...
        feed = {
            ...
            #print(word_ids, sequence_lengths)
        }

        if self.config.chars:
            feed[self.char_ids] = char_ids
            feed[self.word_lengths] = word_lengths
            #print('char_ids, word_lengths :', char_ids, word_lengths)
    ...
    def predict_batch(self, sess, words):
        ...
        if self.config.crf:
            ...
            logits, transition_params = sess.run([self.logits, self.transition_params],
                                                 feed_dict=fd)
            #print('TF Duration = {0}'.format(time.time() - duration))
            #duration = time.time()
            ...
            #print('CRF Duration = {0}'.format(time.time() - duration))
